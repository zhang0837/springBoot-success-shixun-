...
public class AppController {
	 ...
	 * 这种方式也能达到重新向带参，而且能隐藏参数，其原理就是放到session中，session在跳到页面后马上移除对象。所以你刷新一下后这个值就会丢掉 例：
	 * attr.addFlashAttribute("status","999");
	 * attr.addFlashAttribute("message","登录失败"); return "redirect:/toLogin";
	 * 这种方式确实能重定向带参，但是有个问题，这个只能重定向到页面上，也就是说重定向的这个页面视图，能用el表达式直接获取你带参的值，比如status
	 * 和message ，都能获取。
	 */
	@RequestMapping(value = "/reg", method = RequestMethod.POST)
	@Transactional
	public String doReg(String realName, String userName, String password, String code,
			@SessionAttribute(SESSION_VERIFY_CODE_KEY) String verifyCode, RedirectAttributes attributes) {
		if (isEmpty(code)) {
		...
		// 如果attribute中有对象（有erro）则重定向到注册界面
		if (attributes.getFlashAttributes().size() > 0) {
			return "redirect:/reg";
			// 否则将用户的注册信息取出封装到Member对象里面
		} else {
	...
	@Transactional
	@ResponseBody
	public AjaxResult doChangePassword(@SessionAttribute(name = "s_member") Member member, String oldPassword,
	...
	public AjaxResult doChangeInfo(@SessionAttribute("s_member") Member smember, Member member, HttpSession session) {
		...
		if (isEmpty(member.getRealName()) || isEmpty(member.getTelephone()) || isEmpty(member.getEmail())
